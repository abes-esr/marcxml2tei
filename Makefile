...
bundle.1.0: rebuild-mapping
	@.script/template.sh template/bundle.1.0.xsl | sed -r 's/ xml:base=".*?"| xmlns:xi="http:\/\/www.w3.org\/2001\/XInclude"//g'

# builds an all-in-one file which embeds the mapping data and the cleaner
bundle.2.0: rebuild-mapping
	@.script/template.sh template/bundle.2.0.xsl | sed -r 's/ xml:base=".*?"| xmlns:xi="http:\/\/www.w3.org\/2001\/XInclude"//g'

# builds an xslt compliant with Oracle
oracle:
	@.script/template.sh template/oracle.xsl | sed -r 's/ xml:base=".*?"| xmlns:xi="http:\/\/www.w3.org\/2001\/XInclude"//g'
